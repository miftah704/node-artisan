...
export function makeMiddleware(name, functions = []) {
  ...
  const folderPath = path.join(__dirname, '..', 'middleware', ...parts)
...
export default ${routeName}`
  ...
  console.log(
    `${chalk.green(figures.pointerSmall)} Class Name  : ${chalk.cyan(pascalClassName)}Route`
  )
  console.log(
    `${chalk.green(figures.pointerSmall)} File Name   : ${chalk.cyan(kebabCaseName)}.middleware.ts`
  )
