...
export function makeController(name, functions = []) {
  ...
  const folderPath = path.join(__dirname, '..', 'controllers', ...parts)
...
export default class ${pascalClassName}Controller {
  ...
  console.log(
    `${chalk.green(figures.pointerSmall)} Class Name  : ${chalk.cyan(pascalClassName)}Controller`
  )
  console.log(
    `${chalk.green(figures.pointerSmall)} File Name   : ${chalk.cyan(kebabCaseName)}.controller.ts`
  )
