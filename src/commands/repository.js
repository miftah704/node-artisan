...
export function makeRepository(name, functions = []) {
  ...
  const folderPath = path.join(__dirname, '..', 'repositories', ...parts)
...
${classFunctions}
  ...
  console.log(
    `${chalk.green(figures.pointerSmall)} Repository Name: ${chalk.cyan(`${pascalClassName}`)}`
  )
  console.log(
    `${chalk.green(figures.pointerSmall)} File Name      : ${chalk.cyan(`${kebabCaseName}.repository.ts`)}`
  )
