...
const divider = chalk.gray(figures.line.repeat(80))
...
export default class LogUtil {
  ...
  static logSummary(pascalClassName, kebabCaseName, folderPath, type) {
    console.log('\n')
    console.log(chalk.bgYellow(chalk.bold(`✨✨✨ SUMMARY ✨✨✨`)))
    console.log(
      `${chalk.green(figures.pointerSmall)} Class Name     : ${chalk.cyan(
        pascalClassName
      )}${Str.title(type)}`
    )
    console.log(
      `${chalk.green(figures.pointerSmall)} File Name      : ${chalk.cyan(
        kebabCaseName
      )}.${Str.lower(type)}.ts`
    )
  ...
  static logFolderCreation(folderPath, isCreated) {
    if (isCreated) {
      ...
      console.log(
        chalk.yellow(`${figures.warning} Folder already exists :`),
        chalk.gray(folderPath)
      )
    }
  }
  ...
  static logFileExistence(filePath) {
    console.log(
      chalk.red(`${figures.cross} File already exists   :`),
      chalk.red.underline(filePath)
    )
    console.log(chalk.gray(figures.line.repeat(80)))
  }
  ...
  static logHelperCreationSuccess(filePath, functions = null) {
    console.log(
      chalk.cyan(`${figures.pointerSmall} File Location  :`),
      chalk.blue.underline(filePath)
    )
    console.log(
      chalk.magenta(`${figures.star} Functions      :`),
      functions && functions.length > 0
        ? chalk.white(functions.map((fn) => chalk.cyan.bold(fn)).join(', '))
        : chalk.gray('No functions specified')
    )
    console.log(chalk.gray(figures.line.repeat(80)))
  }
}
