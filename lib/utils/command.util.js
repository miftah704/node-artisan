...
class Command {
  ...
  static toKebabCase(str) {
    return str.replace(/([a-z])([A-Z])/g, '$1-$2') // Add dash between camelCase
  ...
  static toPascalCase(str) {
    return str.replace(/[^a-zA-Z0-9]+(.)/g, (_, chr) => chr.toUpperCase()).replace(/^(.)/, (_, chr) => chr.toUpperCase());
  }
  ...
  static toCamelCase(str) {
    return str.replace(/[^a-zA-Z0-9]+(.)/g, (_, chr) => chr.toUpperCase()).replace(/^(.)/, (_, chr) => chr.toLowerCase());
  }
