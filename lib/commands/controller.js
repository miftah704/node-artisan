...
function makeController(name) {
  ...
  var controllerName = parts.pop();
  var folderPath = _path["default"].join.apply(_path["default"], [_dirname, '..', 'controllers'].concat(_toConsumableArray(parts)));
  var fileName = _commandUtils["default"].removeSuffixFromName(controllerName, 'controller');
  ...
  var filePath = _path["default"].join(folderPath, "".concat(kebabCaseName, ".controller.ts"));
  var classFunctions = _commandUtils["default"].generateFunctions(functions, fileName);
  var content = "import { NextFunction, Request, Response } from 'express'\nexport default class ".concat(pascalClassName, "Controller {\n  ").concat(classFunctions, "\n}");
  var divider = _chalk["default"].gray(_figures["default"].line.repeat(60));
  console.log(_chalk["default"].bold.blue("\n".concat(_figures["default"].info, " [Controller Generator]\n")));
  console.log(_chalk["default"].bold("\u2728 Summary:"));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " Class Name  : ").concat(_chalk["default"].cyan(pascalClassName), "Controller"));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " File Name   : ").concat(_chalk["default"].cyan(kebabCaseName), ".controller.ts"));
  ...
  console.log(_chalk["default"].green("".concat(_figures["default"].tick, " Controller created successfully!")));
  ...
  console.log(_chalk["default"].magenta("".concat(_figures["default"].star, " Functions:")), functions.length > 0 ? _chalk["default"].white(functions.map(function (fn) {
    return _chalk["default"].cyan.bold(fn);
  }).join(', ')) : _chalk["default"].gray('No functions specified'));
  console.log(divider);
}