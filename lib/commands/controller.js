...
export default class ${pascalClassName}Controller {
  ...
  if (options.service || options.all) {
    console.log(chalk.italic(chalk.dim(`${figures.info} Generating service for controller: ${fileName}`)));
    makeService(name);
  }
  if (options.repository || options.all) {
    console.log(chalk.italic(chalk.dim(`${figures.info} Generating repository for controller: ${fileName}`)));
    makeRepository(name);
  }
}