...
function makeService(name) {
  ...
  var folderPath = _path["default"].join.apply(_path["default"], [_dirname, '..', 'services'].concat(_toConsumableArray(parts)));
  var fileName = _commandUtils["default"].removeSuffixFromName(serviceName, 'service');
  var kebabCaseName = _commandUtils["default"].toKebabCase(fileName);
  var filePath = _path["default"].join(folderPath, "".concat(kebabCaseName, ".service.ts"));
  var classFunctions = _commandUtils["default"].generateFunctions(functions, fileName);
  var content = "export default class ".concat(fileName, "Service {\n").concat(classFunctions, "\n}");
  var divider = _chalk["default"].gray(_figures["default"].line.repeat(60));
  console.log(_chalk["default"].bold.blue("\n".concat(_figures["default"].info, " [Service Generator]\n")));
  console.log(_chalk["default"].bold("\u2728 Summary:"));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " Service Name  : ").concat(_chalk["default"].cyan("".concat(fileName))));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " File Name     : ").concat(_chalk["default"].cyan("".concat(kebabCaseName, ".service.ts"))));
  ...
  console.log(_chalk["default"].green("".concat(_figures["default"].tick, " Service created successfully!")));
