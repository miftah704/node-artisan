...
function makeTransformer(name) {
  var parts = name.split('/');
  var transformerName = parts.pop();
  var folderPath = _path["default"].join.apply(_path["default"], [_dirname, '..', 'transformers'].concat(_toConsumableArray(parts)));
  var fileName = _commandUtils["default"].removeSuffixFromName(transformerName, 'transformer');
  ...
  var filePath = _path["default"].join(folderPath, "".concat(kebabCaseName, ".transformer.ts"));
  var relativePath = _path["default"].relative(folderPath, _path["default"].join(_dirname, '..', 'transformers'));
  var content = "import Transformer from '".concat(relativePath, "/transformer.util.js'\nimport Str from '").concat(relativePath, "/utils/str.util.js'\n\nexport default class ").concat(fileName, "Transformer extends Transformer {\n  static override format(data: any, lang?: string): Record<string, any> | null {\n    return Str.attributes({\n      // Transformer data here\n    })\n  }\n}");
  var divider = _chalk["default"].gray(_figures["default"].line.repeat(60));
  console.log(_chalk["default"].bold.magenta("\n".concat(_figures["default"].info, " [Transformer Generator]\n")));
  console.log(_chalk["default"].bold("\u2728 Summary:"));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " Transformer Name: ").concat(_chalk["default"].cyan("".concat(fileName, "Transformer"))));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " File Name       : ").concat(_chalk["default"].cyan("".concat(kebabCaseName, ".transformer.ts"))));
  ...
  console.log(_chalk["default"].green("".concat(_figures["default"].tick, " Transformer created successfully!")));
