...
function makeMiddleware(name) {
  ...
  var middlewareName = parts.pop();
  var folderPath = _path["default"].join.apply(_path["default"], [_dirname, '..', 'middleware'].concat(_toConsumableArray(parts)));
  var fileName = _commandUtils["default"].removeSuffixFromName(middlewareName, 'middleware');
  ...
  var filePath = _path["default"].join(folderPath, "".concat(kebabCaseName, ".middleware.ts"));
  var classFunctions = _commandUtils["default"].generateFunctions(functions, fileName);

  // Create the dynamic route name
  var routeName = "".concat(fileName, "Route");
  var content = "import express from 'express'\nconst ".concat(routeName, " = express()\n// Middleware functions can be added here\nexport default ").concat(routeName);
  var divider = _chalk["default"].gray(_figures["default"].line.repeat(60));
  console.log(_chalk["default"].bold.blue("\n".concat(_figures["default"].info, " [middleware Generator]\n")));
  console.log(_chalk["default"].bold("\u2728 Summary:"));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " Class Name  : ").concat(_chalk["default"].cyan(pascalClassName), "Route"));
  console.log("".concat(_chalk["default"].green(_figures["default"].pointerSmall), " File Name   : ").concat(_chalk["default"].cyan(kebabCaseName), ".middleware.ts"));
  ...
  console.log(_chalk["default"].green("".concat(_figures["default"].tick, " middleware created successfully!")));
