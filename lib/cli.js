...
program.command('list').description('List all available artisan commands').action(() => {
  ...
  ...program.commands.filter(cmd => cmd.name().startsWith('make:')).map(cmd => ({
    name: cmd.name(),
    description: `${cmd.description()}` + (cmd.name() === 'make:model' ? '\n' + chalk.gray('Options:') + '\n' + chalk.green('  -c, --controller') + ': Create a controller for the model' + '\n' + chalk.green('  -a, --all') + ': Create a controller, service, and repository for the model' : cmd.name() === 'make:controller' ? '\n' + chalk.gray('Options:') + '\n' + chalk.green('  -s, --service') + ': Create a service for the controller' + '\n' + chalk.green('  -r, --repository') + ': Create a repository for the controller' + '\n' + chalk.green('  -a, --all') + ': Create a service and repository for the controller' : '')
  }))];
  ...
  const config = {
    columns: {
      0: {
        alignment: 'left',
        width: Math.min(20, Math.floor(maxWidth * 0.25))
      },
      // 25% for the first column
      1: {
        alignment: 'left',
        width: Math.min(80, Math.floor(maxWidth * 0.75))
      } // 75% for the second column
    },
    border: {
      topBody: chalk.gray('─'),
      topJoin: chalk.gray('┬'),
  ...
  // Create table rows
  const rows = [[chalk.bold('Command'), chalk.bold('Description')], ...commands.map(cmd => [chalk.bold.yellow(cmd.name), cmd.description])];
...
program.command('make:controller <name> [functions...]').description('Create a new controller in src/controllers directory').option('-s, --service', 'Create a service for the controller').option('-r, --repository', 'Create a repository for the controller').option('-a, --all', 'Create service and repository for the controller').action((name, functions = [], options) => {
...
program.command('make:model <name>').description('Create a new model in src/models directory').option('-c, --controller', 'Create a controller for the model').option('-a, --all', 'Create all related files (controller, service, repository)').action((name, options) => {
...
program.command('make:route <name>').description('Create a new route in src/routes directory').action(name => makeRoute(name));
